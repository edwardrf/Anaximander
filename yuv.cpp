...
void YUYVtoBGR(int width, int height, unsigned char* src, unsigned char* dst) {
	...
	for (i = 0; i < width * height / 2; ++i) {
		...
		*tmp++ = (unsigned char)YUV2B(*py1, *pu, *pv);
		*tmp++ = (unsigned char)YUV2G(*py1, *pu, *pv);
		*tmp++ = (unsigned char)YUV2R(*py1, *pu, *pv);
		*tmp++ = (unsigned char)YUV2B(*py2, *pu, *pv);
		*tmp++ = (unsigned char)YUV2G(*py2, *pu, *pv);
		*tmp++ = (unsigned char)YUV2R(*py2, *pu, *pv);
	}
}
